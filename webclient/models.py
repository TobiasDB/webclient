...
``scripts/gen_stubs.py`` from the op contract -- the same generator that emits
the eager surface blocks -- and gated by ``--check``. (Deriving that contract
from the backing signatures, rather than the declared op table, is the next
step.)
"""
