...
- `query/` -- the lazy query layer: `expr.py` (the `Expr` recorder + the `wq`
  authoring namespace), `plan.py` (the serialisable `Plan` IR -- the wire form
  for the service/remote), and `executor.py` (one async walk over a `Plan`).
