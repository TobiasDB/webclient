"""The core IS the eager client (option B): a ``WebClientCore`` is directly a
...
from webclient.core.client import WebClientCore
...
def test_core_is_a_context_managed_eager_client(site):
    with WebClientCore() as c:
        doc = c.fetch(site.url_for("/"))  # eager -> a Document (its core)
        assert isinstance(doc, DocumentCore) and doc.ok
        assert doc.select("h1").text_content == "Hi"  # dispatch on the resolved core
        assert isinstance(doc.select("a").attr("href"), ReferenceCore)  # href narrows
...
def test_eager_ref_is_a_reference_core(site):
    with WebClientCore() as c:
        ...
        assert isinstance(ref, ReferenceCore)
